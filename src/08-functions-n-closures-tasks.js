 ...
 *   getPolynom()      => null
 */
const getPolynom = (...nums) => (x) => {
  if (nums.length > 2) return nums[0] * x ** 2 + nums[1] * x + nums[2];
  if (nums.length > 1) return nums[0] * x + nums[1];
  return nums[0];
};
...
function logger(func, logFunc) {
  return function fn(...args) {
    const strLog = `${func.name}(${JSON.stringify(args).slice(1, -1)})`;
